...
def load_data(spark, filepath):
    ...
    try:
        # Используем file:// для указания локального файла
        df = spark.read.csv(f"file://{filepath}", header=True, inferSchema=True)
        return df
...
def main():
    ...
    # Проверим что файл скопировался локально
    if not os.path.exists(local_temp_path):
        print(f"Ошибка: Локальный файл не создан: {local_temp_path}")
        spark.stop()
        sys.exit(1)

    print(f"Файл скопирован из HDFS, размер: {os.path.getsize(local_temp_path)} байт")
